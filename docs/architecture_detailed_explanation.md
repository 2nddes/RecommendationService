...
- app/reco/factory.py
  ...
  - 当 `config.json.tag_recall.enabled=true` 时，会额外注入 `tag_inverted` 召回（Redis 倒排：genre + director）。
- app/reco/recall/
  - 召回能力（主实现为 two_tower，可选叠加 tag_inverted）。
- app/reco/ranking/
  - 排序能力（当前主实现为 mmoe）。
- app/reco/reranking/
  - 重排能力（当前实现随机扰动策略）。
...
- app/reco/startup.py
  - 启动后后台构建与周期刷新任务。
  - 缓存预计算会在同一 worker 中刷新 `features` / `trending` / `tag_recall`。
