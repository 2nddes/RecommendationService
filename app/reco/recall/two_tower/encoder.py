...
class FeatureTwoTowerEncoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        stats_dim: int,
        seed: int,
        enable_deep_encoder: bool = True,
        deep_hidden_mult: int = 2,
        deep_dropout: float = 0.10,
    ) -> None:
        super().__init__()
        ...
        self.profession_table = nn.Embedding(profession_bucket_size + 1, dim)
        self.tag_table = nn.Embedding(tag_count, dim)
        self.item_stats_proj = nn.Linear(stats_dim, dim)
        self.profession_proj = nn.Linear(dim, dim)
        self.user_proj = nn.Linear(dim * 5, dim)
        self.item_proj = nn.Linear(dim * 3, dim)

        hidden_dim = max(int(dim) * max(int(deep_hidden_mult), 1), int(dim))
        self.user_deep = None
        self.item_deep = None
        if bool(enable_deep_encoder):
            self.user_deep = nn.Sequential(
                nn.Linear(dim, hidden_dim),
                nn.GELU(),
                nn.Dropout(float(deep_dropout)),
        ...
        with torch.no_grad():
            ...
            self.profession_table.weight.normal_(mean=0.0, std=0.05, generator=gen)
    ...
    def encode_user_inputs(
        ...
        *,
        ...
        profession_vec = self.profession_table(profession_idx)
        ...
        user_fused = self.user_proj(user_input) + self.profession_proj(profession_vec)
        if self.user_deep is not None:
            user_fused = user_fused + self.user_deep(user_fused)
        return F.normalize(user_fused, p=2, dim=1, eps=1e-12)
    ...
    def encode_item_inputs(
        ...
        *,
        ...
        item_fused = self.item_proj(item_input)
        if self.item_deep is not None:
            item_fused = item_fused + self.item_deep(item_fused)
        return F.normalize(item_fused, p=2, dim=1, eps=1e-12)
    ...
    def forward(
        ...
        *,
        ...
        user_profession_idx: torch.Tensor,
